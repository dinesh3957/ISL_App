...
def translate(marathi_text: str, pdf_file):
    ...
    try:
        if model_type == "ct2":
            ...
            for s in sentences:
                inputs = tokenizer(s, return_tensors="pt", padding=True, truncation=True, max_length=256).to(DEVICE)
                with torch.inference_mode():
        ...
        status_msg  = f"✅ Done in {elapsed:.1f}s · {word_count} words · {model_label} · {DEVICE.upper()}"
...
with gr.Blocks(css=CUSTOM_CSS, title="Marathi → English Translator", theme=gr.themes.Base()) as demo:
